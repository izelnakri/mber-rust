...
pub fn generate(input_name: String, application_name: &str, project_root: PathBuf) -> io::Result<()> {
    let target_folder = format!("{}/src/ui/routes/{}", project_root.to_str().unwrap(), input_name);
    ...
    utils::write_file_if_not_exists(
        ...
        get_acceptance_test_code(input_name.clone(), application_name).as_str(),
        &project_root
    )?;

    return utils::write_file_if_not_exists(
        format!("{}/unit-test.js", &target_folder),
        get_unit_test_code(input_name, application_name).as_str(),
        &project_root
    );
}

fn get_acceptance_test_code(input_name: String, application_name: &str) -> String {
...
module('Acceptance | {}', function(hooks) {{
  ...
  // }});
}});", application_name, input_name);
}

fn get_unit_test_code(input_name: String, application_name: &str) -> String {
  return format!("import {{ module, test }} from 'qunit';
