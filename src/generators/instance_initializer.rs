...
const INSTANCE_INITIALIZER_CODE: &'static str = "export function initialize(/* appInstance */) {
...
pub fn generate(input_name: String, application_name: &str, project_root: PathBuf) -> io::Result<()> {
    let target_folder = format!("{}/src/init/instance-initializers", project_root.to_str().unwrap());
    let target_file_path = format!("{}/{}", target_folder, input_name);

    fs::create_dir_all(target_folder)?;
    utils::write_file_if_not_exists(format!("{}.js", target_file_path), INSTANCE_INITIALIZER_CODE, &project_root)?;
...
module('Unit | Instance Initializer | {}', function(hooks) {{
  setupTest(hooks);
