...
function getFileName(environment, options) {
  if (options.excludeEmberData) {
    return environment === "production"
      ? "no-ember-data-ember-prod"
      : "no-ember-data-ember-debug";
  }
...
async function readBuildFiles(
  projectPath,
  environment,
  options = { excludeEmberData: false }
) {
  ...
  let baseBuilds = [
    ...
    importAddonFolderToAMD(
      "@glimmer/resolver",
      "@glimmer/resolver/dist/modules/es2017"
    ),
  ...
  if (!options.excludeEmberData) {
    baseBuilds = baseBuilds.concat(
      await buildEmberData(projectPath, environment)
    );
  }
...
async function buildEmberData(projectPath, environment) {
  const emberDataVersion = (await import(`${projectPath}/package.json`)).default
  ...
  process.env.EMBER_ENV =
    environment === "production" ? "production" : undefined; // NOTE: hack for hacky ember-data builds
